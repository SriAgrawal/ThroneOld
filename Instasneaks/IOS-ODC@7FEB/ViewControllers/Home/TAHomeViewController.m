...
//  Throne App
//
//  Created by Shridhar Agarwal on 23/12/16.
...
@interface TAHomeViewController ()<UICollectionViewDelegate,UICollectionViewDataSource,UITextFieldDelegate,BIZSliderViewDelegate,navigationDelegateForFilter,navigationDelegateForProductDetails,navigationDelegateForSkipLogin>
{
...
@property (strong, nonatomic) NSMutableDictionary    * dictOfFilters;
...
-(void)initialSetup{
    ...
    self.dictOfFilters = [NSMutableDictionary new];
    ...
    if (self.isfromOnboard) {
        [self performSelector:@selector(pushCategoryController) withObject:nil afterDelay:0.0];
    }
    //    [self makeApiCallToGetTheCategoryList];
}
...
-(void)textFieldDidBeginEditing:(UITextField *)textField{
    ...
    [searchResultContainerVC setIsForProduct:YES];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    headerSectionLbl.text = [@[@"FEATURED PRODUCTS",@"TRENDING STORES",@"TRENDING PRODUCTS"] objectAtIndex:section];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (indexPath.section == 1) {
        ...
        productDetailVC.isCommingFromService = NO;
...
- (IBAction)filterButtonAction:(UIButton *)sender {
    ...
    filterVC.categoryPassedArray = self.categoryDataArray;
    filterVC.filterDict  = self.dictOfFilters;
...
- (void)manageTheNavigation:(NSMutableArray*)arrayForFilterObj filterPreference:(TAFilterInfo *)filterObj{
...
-(void)makeApiCallToGetProductList{
    ...
    [[ServiceHelper helper] request:nil apiName:@"products?featured=1" withToken:YES method:GET onViewController:self completionBlock:^(id result, NSError *error) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            [self.dictOfFilters setValue:[result valueForKey:@"filters"] forKey:@"filters"];
            [self makeApiCallToGetTrendingProductList];
        });
    }];
}
