...
@interface TAAllSearchResultsVC ()<UITextFieldDelegate, UITableViewDelegate, UITableViewDataSource>{
...
@implementation TAAllSearchResultsVC
...
-(void)setUpDefaults{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(refreshAllSearchResult:) name:RefreshAllSearchedResultData object:nil];
}
...
-(void)setSectionTitleWithText:(NSString *)text{
    ...
    self.sectionsTitleArray = [NSMutableArray arrayWithObjects:[NSString stringWithFormat:@"%@ SEARCHES", text], [NSString stringWithFormat:@"%@ ITEMS", text], [NSString stringWithFormat:@"PROFILES WITH %@", text], nil];
    [self makeApiCallToGetSearchedListWithQuery:[NSString stringWithFormat:@"search_queries?query=%@%@", [[UserInfo sharedManager] searchText], categoryQuery]];
}

-(void)refreshAllSearchResult:(NSNotification *)object{
    [self setSectionTitleWithText:[[UserInfo sharedManager] searchText]];
}
...
-(void)showMoreBtnAction:(UIButton *)sender{
    ...
    switch (sender.tag%sectionButtonTag) {
        case 0:
        {
            [self makeApiCallToGetSearchedListWithQuery:[NSString stringWithFormat:@"search_queries?query=%@%@?page=%ld", [[UserInfo sharedManager] searchText], categoryQuery, ([self.searchesPagination.current_page integerValue] + 1)]];
        }
            break;
        ...
        default:{
            ...
            [self makeApiCallToGetSearchedStoresListWithQuery:[NSString stringWithFormat:@"vendors?product_name=%@%@?page=%ld", [[[UserInfo sharedManager] searchText] lowercaseString], categoryQuery, ([self.storesPagination.current_page integerValue] + 1)]];
        }
            break;
    }
}
...
-(NSMutableAttributedString *)getAttributedStringForSection:(NSInteger)section{
    ...
    [attributeString addAttribute:NSForegroundColorAttributeName value:[UIColor blackColor] range:(NSRange){((section == 2) ? 14 : 0), [serachedText length]}];
...
-(void)dealloc{
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:RefreshAllSearchedResultData object:nil];
}
