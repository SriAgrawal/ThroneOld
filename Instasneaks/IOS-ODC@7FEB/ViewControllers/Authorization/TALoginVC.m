...
@interface TALoginVC ()<TTTAttributedLabelDelegate,navigationDelegateForSignUp>{
    UserInfo* userInfo;
    NSInteger errorTag;
...
- (void)manageTheNavigation:(UIViewController*)isFromViewController{
    ...
    [self dismissViewControllerAnimated:NO completion:^{
        if(_delegateForLogin && [_delegateForLogin respondsToSelector:@selector(manageTheNavigation:)])
        {
            [_delegateForLogin manageTheNavigation:self];
        }
    }];
