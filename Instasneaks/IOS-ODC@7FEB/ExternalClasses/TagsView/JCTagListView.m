...
- (void)setup {
    ...
    _tagTextLightFont = [AppUtility sofiaProLightFontWithSize:12];
    _tagTextBoldFont = [AppUtility sofiaProBoldFontWithSize:12];
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CGRect frame = [self.tags[indexPath.item] boundingRectWithSize:maxSize options:NSStringDrawingUsesFontLeading|NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: ((self.tags.count-1 == indexPath.item) ? self.tagTextBoldFont : self.tagTextLightFont)} context:nil];
    
    return CGSizeMake(frame.size.width, layout.itemSize.height);
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    NSMutableAttributedString *attributeString = [[NSMutableAttributedString alloc] initWithString:self.tags[indexPath.item]];
    ...
    cell.titleLabel.font = self.tagTextLightFont;
    if (self.tags.count-1 == indexPath.item) {
        cell.titleLabel.font = self.tagTextBoldFont;
