...
NSString * const kTWMessageBarStyleSheetImageIconError = @"small-x-white.png";
...
+ (void)initialize
{
	if (self == [TWMessageView class])
	{
        ...
        kTWMessageViewDescriptionFont = [AppUtility sofiaProLightFontWithSize:14];
    ...
    if ([self.delegate respondsToSelector:@selector(styleSheetForMessageView:)])
    {
        ...
        CGContextSaveGState(context);
        {
            if ([styleSheet respondsToSelector:@selector(iconImageForMessageType:)])
            {
                ...
                [[styleSheet iconImageForMessageType:self.messageType] drawInRect:CGRectMake(windowWidth-30, yOffset-3, kTWMessageViewIconSize, kTWMessageViewIconSize)];
            }
        }
        CGContextRestoreGState(context);
...
- (CGFloat)height
{
   // CGSize titleLabelSize = [self titleSize];
    CGSize descriptionLabelSize = [self descriptionSize];
    return MAX((kTWMessageViewBarPadding * 2) + descriptionLabelSize.height + [self statusBarOffset], (kTWMessageViewBarPadding * 2) + kTWMessageViewIconSize + [self statusBarOffset]);
}
...
+ (void)initialize
{
	...
	{
        // Colors (background)
        kTWDefaultMessageBarStyleSheetErrorBackgroundColor = [UIColor redColor]; // orange
