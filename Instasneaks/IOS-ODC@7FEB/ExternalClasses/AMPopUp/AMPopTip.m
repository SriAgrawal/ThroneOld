...
- (void)setup {
    ...
    _textBounds.origin = (CGPoint){self.padding + self.edgeInsets.left, self.padding + self.edgeInsets.top + 10};
...
//----------To increase popover height
    if (self.direction == AMPopTipDirectionUp || self.direction == AMPopTipDirectionDown) {
        frame.size = (CGSize){self.textBounds.size.width + self.padding * 2.0 + self.edgeInsets.left + self.edgeInsets.right, 160 + self.arrowSize.height};
        ...
        //Added controller component (Optional)
        UILabel *lineLabel = [[UILabel alloc]initWithFrame:CGRectMake(15, self.textBounds.size.height + 36, self.textBounds.size.width + self.padding * 2.0 + self.edgeInsets.left-20, 2)];
        lineLabel.backgroundColor = [UIColor blackColor];
        [self addSubview:lineLabel];

        UIButton *crossButton;
        if(self.isCenter) {
          crossButton = [[UIButton alloc]initWithFrame:CGRectMake(130, self.textBounds.size.height + 60, 63, 22)];
        }
        else {
          crossButton = [[UIButton alloc]initWithFrame:CGRectMake(245, self.textBounds.size.height + 60, 63, 22)];
        }
        [crossButton setTitle:@"DOPE" forState:UIControlStateNormal];
        [crossButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        [crossButton setImage:[UIImage imageNamed:@"small-x"] forState:UIControlStateNormal];
        [crossButton setImageEdgeInsets:UIEdgeInsetsMake(4, 70, 5, 0)];
        crossButton.titleLabel.font = [UIFont fontWithName:@"SofiaPro-Bold" size:13.0f];
...
-(void)showPopOverWithRightButton : (nonnull UIButton *)button{
